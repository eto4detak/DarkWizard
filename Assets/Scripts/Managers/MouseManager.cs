...
public class MouseManager : Singleton<MouseManager>
{
    ...
    private List<Unit> selectedGroup = new List<Unit>();
    ...
    private void TrySelectUnit()
    {
        ...
        if (mouseHit.collider == null)
        {
            ...
            if (units.Length > 0)
            {
                selected = UnityExtension.GetClosest(mouseHit.point, PController.instance.playerUnits) as Unit;
            }
            else return;
        }
        else
        {
            selected = mouseHit.collider.GetComponent<Unit>();
        }
